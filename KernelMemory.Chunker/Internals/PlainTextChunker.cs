...
/// </summary>
internal sealed class PlainTextChunker
{
    ...
    public PlainTextChunker(string tokenizer = "cl100k_base")
    {
        this._tokenizer = Microsoft.ML.Tokenizers.TiktokenTokenizer.CreateForEncoding(tokenizer);
    }
    ...
    public List<string> Split(string text, PlainTextChunkerOptions options)
    {
        ...
        if (options.Overlap > 0 && chunks.Count > 1)
        {
            ...
            for (int index = 1; index < chunks.Count; index++)
            {
                ...
                var previousChunkTokens = this.GetTokens(chunks[index - 1]);
                var overlapTokens = previousChunkTokens.Skip(previousChunkTokens.Count - options.Overlap);
        ...
        ref bool firstChunkDone)
    {
        ...
        if (this.TokenCount(text) <= maxChunkSize)
        {
            return [text];
        }
